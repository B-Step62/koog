...
Koog provides two main types of prompt executors:

| Type            | Description                                                                                                                                                                                                                                                          |
|-----------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Single-provider | Wraps a single LLM client for one provider. Use this executor if your agent only requires switching between models within a single LLM provider.                                                                                                                     |
| Multi-provider  | Wraps multiple LLM clients and routes calls based on the LLM provider. It can optionally use a configured fallback provider and LLM when the requested client is unavailable. Use this executor if your agent needs to switch between LLMs from different providers. |

<!--TODO: Update this section once the PromptExecutor class is implemented. This class will aacept 1 or multiple LLM clients to create
the correponding prompt executor (single- or multi-provider).
Currently, the MultiLLMPromptExecutor class should be used for both single- and multi-provider executors.-->
