...
=== "Java"
    ...
    ```java
    // Java-oriented construction of an agent with tools typically uses the builder API.
    // However, agentStrategy here is a Kotlin DSL object and cannot be supplied from Java.
    // ToolRegistry has a Builder for Java usage.

    ToolRegistry registry = ToolRegistry.builder()
        // .tool(new BookTool())
        // FAILED: BookTool extends Kotlin SimpleTool with suspend execute; a Java implementation isn’t available.
        .build();

    AIAgent<String, String> agent = AIAgent.builder()
        // .promptExecutor(simpleOpenAIExecutor(token))
        // FAILED: simpleOpenAIExecutor is a Kotlin top-level helper; Java would need a client/executor exposed for Java.
        .toolRegistry(registry)
        // .strategy(agentStrategy)
        // FAILED: agentStrategy is defined via Kotlin DSL and not constructible in Java directly.
        .build();
    ```
